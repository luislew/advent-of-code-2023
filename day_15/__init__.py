...
starting from the point where light enters the facility. The boxes have holes that allow light to pass
from one box to the next all the way down the line.
...
Inside each box, there are several lens slots that will keep a lens correctly positionedto focus light passing through
the box. The side of each box has a panel that opens to allow you to insert or remove lenses as necessary.

Along the wall running parallel to the boxes is a large library containing lenses organized by focal length
ranging from 1 through 9. The reindeer also brings you a small handheld label printer.
...
already in the box. Don't move any of the other lenses when you do this. If there aren't any lenses in the box,
...
class LightBoxes:
    ...
    def add_lens(self, label, focal_length):
        ...
        matching_lens = [i for i, (l, _) in enumerate(box) if l == label]
        if matching_lens:
            box[matching_lens[0]] = (label, focal_length)
        else:
    ...
    def process_step(self, step):
        if "=" in step:
            ...
            self.remove_lens(step.replace("-", ""))

    def get_focusing_power(self):
        return sum(
            (box_id + 1) * (i + 1) * focal_length
