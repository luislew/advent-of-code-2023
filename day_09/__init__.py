...
def determine_next_value(sequence):
    reductions = [sequence]
    while any(reductions[-1]):
        seq = reductions[-1]
        reductions.append([seq[i + 1] - seq[i] for i in range(len(seq) - 1)])

    # work inside out to determine the next value
    for i in range(-2, -len(reductions) - 1, -1):
        reduction = reductions[i]
        reduction.append(reductions[i + 1][-1] + reduction[-1])

    return reductions[0][-1]


def determine_previous_value(sequence):
    reductions = [sequence]
    while any(reductions[-1]):
        seq = reductions[-1]
        reductions.append([seq[i + 1] - seq[i] for i in range(len(seq) - 1)])

    # work inside out to determine the previous value
    for i in range(-2, -len(reductions) - 1, -1):
        reduction = reductions[i]
        reduction.insert(0, reduction[0] - reductions[i + 1][0])
